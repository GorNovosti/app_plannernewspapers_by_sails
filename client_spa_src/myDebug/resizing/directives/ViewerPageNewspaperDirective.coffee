...
define [
	...
	#	return $(element)
        ...
        @._toGrid = (x)->
            ...
            step =$scope.stepVal
            eps = $scope.stepVal/2
            nearestGridLine = Math.round(x/$scope.step)*$scope.step
            console.log nearestGridLine,nearestX
            return (if Math.abs(x-nearestX)<eps then nearestGridLine else x)
        ...
        $scope.nodeMouseDown = (evt, node) ->
            ...
            dragging.startDrag evt,
                ...
                dragEnded: ->
                    ...
                    step =$scope.stepVal
                    #                    eps = $scope.stepVal/2
                    #                    nearestGridLine = Math.round(node.x/step)*step
                    #                    console.log 'dragEnded',$scope.stepVal,nearestGridLine
                    #                    node.x =  (if Math.abs(node.x)<eps then nearestGridLine else node.x) #Math.round((node.x/step)*step)#-$scope.stepValcontroller._toGrid(node.x)#
                    #                    node.y =  Math.round((node.y/step)*step)#-$scope.stepVal
